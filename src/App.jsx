...
const CATS = ["Destination","Celebrity","Love Story","Wedding Film","Short Film","InstaFilm","Pre Wed","Fashion","BTS","Podcast"];
...
.corm { font-family:'Marcellus',serif; }
...
.loader-brand {
  ...
  animation:brandReveal 1.2s cubic-bezier(.2,.8,.2,1) .15s forwards;
}
.loader-logo-text {
...
.loader-logo-text span { color:${GOLD}; }
.loader-badge {
  ...
  animation:badgeReveal .75s ease .85s forwards;
}
.loader-sub {
  margin-top:22px;
  font-size:10px;
  ...
  opacity:0;
  animation:subReveal .8s ease 1.05s forwards, pulse 1.8s ease-in-out 1.8s infinite;
}
...
.loader-bar-wrap {
  ...
  overflow:hidden;
  opacity:0;
  animation:subReveal .8s ease 1.15s forwards;
}
.loader-bar {
  height:100%;
...
.fg-text h3 { font-family:'Marcellus',serif; font-size:20px; font-weight:400; color:#fff; margin-bottom:5px; }
...
.pin-title { font-family:'Marcellus',serif; font-size:28px; font-weight:400; color:#fff; margin-bottom:8px; }
...
.pin-d {
  ...
  border-radius:10px;
  color:#fff; font-family:'Marcellus',serif;
...
.pin-cancel {
  ...
  letter-spacing:.1em; cursor:pointer; font-family:'Manrope',sans-serif;
  transition:color .2s;
}
...
.modal-title { font-family:'Marcellus',serif; font-size:26px; font-weight:400; color:#fff; margin-bottom:11px; }
...
.ep-n { font-family:'Marcellus',serif; font-size:17px; font-weight:300; color:rgba(255,255,255,.2); width:22px; flex-shrink:0; text-align:center; }
...
.srch-inp {
  ...
  font-family:'Marcellus',serif; font-size:34px;
...
.adm-title { font-family:'Marcellus',serif; font-size:24px; font-weight:400; color:#fff; letter-spacing:.06em; }
...
.pin-field { font-size:18px; letter-spacing:.3em; text-align:center; font-family:'Marcellus',serif; }
...
.yt-fetch {
  ...
  border-radius:8px; color:${GOLD_L};
  font-family:'Manrope',sans-serif; font-size:10px; font-weight:700;
  letter-spacing:.13em; text-transform:uppercase; cursor:pointer;
  transition:all .2s; margin-top:9px;
}
.yt-fetch:hover { background:rgba(201,168,76,.2); border-color:rgba(201,168,76,.5); }
...
.ev-delete {
  ...
  border-radius:100px; color:rgba(224,112,112,.85);
  padding:6px 12px; font-family:'Manrope',sans-serif;
  font-size:9px; font-weight:700; letter-spacing:.13em;
  text-transform:uppercase; cursor:pointer; flex-shrink:0; transition:background .2s;
}
.ev-delete:hover { background:rgba(224,112,112,.18); }
...
@media(max-width:768px){
  ...
  .loader-brand { flex-direction:column; gap:10px; }
  .loader-logo-text { letter-spacing:.18em; }
...
/* ─── ADMIN ───────────────────────────────────────────────────────────────── */
function Admin({ onClose, onAdd, onUpdate, onDelete, events }) {
  ...
  const fetchYoutube=async idx=>{
    const input = form.episodes[idx]?.youtubeId;
    if(!input?.trim()){flash("❌ Add a YouTube link first.");return;}
    try{
      const res = await fetch(`${API_YOUTUBE}?url=${encodeURIComponent(input)}`);
      const data = await res.json();
      if(!res.ok) throw new Error(data.error);
      setForm(f=>{
        const episodes = [...f.episodes];
        episodes[idx] = {
          ...episodes[idx],
          youtubeId: data.youtubeId || episodes[idx].youtubeId,
          title: data.title || episodes[idx].title,
          duration: data.duration || episodes[idx].duration
        };
        return {...f, thumb: f.thumb || data.thumbnail || "", episodes};
      });
      flash("✦ YouTube details fetched.");
    }catch(error){
      flash(`❌ ${error.message || "Could not fetch YouTube details."}`);
    }
  };
  ...
  const submit=async()=>{
    ...
    try{
      if(eid){await onUpdate(data);flash("✦ Event updated.");}
      else{await onAdd(data);flash("✦ Event added to HCM TV.");}
      setTimeout(()=>{setEid(null);setForm(EF);},2700);
    }catch(error){
      flash(`❌ ${error.message || "Could not save event."}`);
    }
  };

  const remove=async ev=>{
    if(!window.confirm(`Delete "${ev.title}" from database?`)) return;
    try{
      await onDelete(ev.id);
      if(eid===ev.id) cancelEdit();
      flash("✦ Event deleted from database.");
    }catch(error){
      flash(`❌ ${error.message || "Could not delete event."}`);
    }
  };
            ...
            <div className="ep-ed">
              {form.episodes.map((ep,i)=>(
                <div className="ep-ed-item" key={ep.id}>
                  ...
                  <button className="yt-fetch" onClick={()=>fetchYoutube(i)}>Fetch Video Title, Duration & Thumbnail</button>
                </div>
              ))}
            ...
            {events.map(ev=>(
              <div className="ev-li" key={ev.id}>
                ...
                <div className="ev-actions">
                  ...
                  <button className="ev-delete" onClick={()=>remove(ev)}>Delete</button>
                </div>
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  const heroVideoId = hero?.episodes?.find(ep=>ep.youtubeId)?.youtubeId;
  const heroOrigin = typeof window !== "undefined" ? encodeURIComponent(window.location.origin) : "";
  ...
  useEffect(()=>{
    ...
    const playHeroVideo=()=>{
      ...
      const send=func=>frame.contentWindow.postMessage(JSON.stringify({
        event:"command",
        func,
        args:[]
    ...
    const t1=setTimeout(playHeroVideo,700);
    const t2=setTimeout(playHeroVideo,1700);
    return()=>{clearTimeout(t1);clearTimeout(t2);};
  },[heroVideoId]);

  const addEvent=async ev=>{
    const saved = await api(API_EVENTS, { method:"POST", body:JSON.stringify(ev) });
    setEvents(e=>[saved,...e.filter(item=>item.id!==saved.id)]);
  };

  const updateEvent=async ev=>{
    const saved = await api(`${API_EVENTS}/${ev.id}`, { method:"PUT", body:JSON.stringify(ev) });
    setEvents(e=>e.map(item=>item.id===saved.id?saved:item));
  };

  const deleteEvent=async id=>{
    await api(`${API_EVENTS}/${id}`, { method:"DELETE" });
      ...
      {loading && (
        <div className={`loader${loadOut?" out":""}`}>
          <div className="loader-brand">
            <div className="loader-logo-text">HCM<span> TV</span></div>
            <span className="loader-badge">Originals</span>
          </div>
          <div className="loader-sub">Premium Films</div>
      ...
      <div className="app">
        ...
        {hero&&(
          <section className="hero">
            ...
            {heroVideoId&&(
              <div className="hero-video" aria-hidden="true">
                <iframe
                  ...
                  src={`https://www.youtube.com/embed/${heroVideoId}?autoplay=1&mute=1&controls=0&playsinline=1&loop=1&playlist=${heroVideoId}&modestbranding=1&rel=0&showinfo=0&enablejsapi=1&origin=${heroOrigin}`}
            ...
            <div className="hero-info-pill">
              ...
              <span className="pill-val" style={{fontFamily:"'Marcellus',serif",fontSize:15}}>{hero.location}</span>
        ...
        <div style={{padding:"0 4vw"}}>
          <div className="stat-bar">
            {[["400+","Films"],["50+","Destinations"],["12","Years"],["12K+","Moments"]].map(([n,l])=>(
        ...
        <footer className="footer">
          <div className="footer-top">
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <img src="/logo-green.png" alt="HCM" style={{height:48,width:48,objectFit:"cover",borderRadius:10,border:"1px solid rgba(100,180,140,.25)"}}/>
              <div>
